...
struct FixedWidthParameters {
  ...
  // 文件末尾是否注明字典string数量 Does the end of the file have the actual strings in the vocabulary?
  bool has_vocabulary;
...
// 二进制头文件中存得参数 Parameters stored in the header of a binary file.
struct Parameters {
...
class BinaryFormat {
  public:
    ...
    // 仅仅读config部分 Used to read parts of the file to update the config object before figuring out full size.
    void ReadForConfig(void *to, std::size_t amount, uint64_t offset_excluding_header) const;
    // 完整加载二进制模型 Actually load the binary file and return a pointer to the beginning of the search area.
