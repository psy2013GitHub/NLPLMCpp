...
namespace detail {
    ...
    /* Model 由 索引类 和 字典类构成, 索引类决定Model类型  */
...
template <class Search, class VocabularyT> GenericModel<Search, VocabularyT>::GenericModel(const char *file, const Config &init_config) : backing_(init_config) {
  util::scoped_fd fd(util::OpenReadOrThrow(file)); // 打开文件
  ...
  begin_sentence.words[0] = vocab_.BeginSentence(); // words 由 </s> 开始
...
template <class Search, class VocabularyT> void GenericModel<Search, VocabularyT>::InitializeFromARPA(int fd, const char *file, const Config &config) {
  ...
  try {
    ...
    if (config.write_mmap && config.include_vocab) {
      ...
      vocab_.ConfigureEnumerate(&wrap, counts[0]); // vocab 只限 1gram
      ...
      vocab_.ConfigureEnumerate(config.enumerate_vocab, counts[0]); // 字典里包含得词就从1 gram里读取
      search_.InitializeFromARPA(file, f, counts, config, vocab_, backing_); // search_ 通过 vocab 和 .arpa文件初始化
    }
