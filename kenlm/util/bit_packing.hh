...
typedef union { float f; uint32_t i; } FloatEnc; // 联合。。。。。
...
const uint32_t kSignBit = 0x80000000; // 取 uint32_t 符号位

inline void SetSign(float &to) { // 负数
  FloatEnc enc;
  enc.f = to;
  enc.i |= kSignBit; // 将最高位置为1
  to = enc.f; // 联合得意义。。。。。。。。。！！！！！
}

inline void UnsetSign(float &to) { // 去除符号
  FloatEnc enc;
  enc.f = to;
  enc.i &= ~kSignBit; // 将最高位置为0
  to = enc.f;
}
