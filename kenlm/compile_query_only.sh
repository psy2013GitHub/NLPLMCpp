...
#rm {lm,util,examples}/*.o 2>/dev/null
set -e
...
objects=""
for i in util/double-conversion/*.cc util/*.cc lm/*.cc examples/* $ADDED_PATHS; do
  if [ "${i%test.cc}" == "$i" ] && [ "${i%main.cc}" == "$i" ]; then # 不以test.cc或main.cc结尾
    echo "$CXX $CXXFLAGS -c $i -o ${i%.cc}.o"
...
$CXX examples/query_server_main.cc $objects -o bin/query_server $CXXFLAGS -std=c++11 -L/opt/boost/lib/ -lboost_filesystem -lboost_regex -lboost_coroutine
